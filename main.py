...
# === AUDIO INPUT ===
# Name of the audio device to listen to. The script looks for this string in device names.
# - macOS: Use "BlackHole" (requires BlackHole driver).
AUDIO_DEVICE_NAME = "BlackHole"

# === WIZ BULB SETTINGS ===
# The standard UDP port used by WiZ bulbs for local control. Do not change unless customized.
WIZ_PORT = 38899

# === AUDIO PROCESSING ===
# Standard audio sampling rate.
SAMPLE_RATE = 44100

# Number of audio samples per frame.
# - 512: Lower latency (~11ms), snappier response, higher CPU usage.
# - 1024: Higher latency (~23ms), smoother, lower CPU usage.
CHUNK_SIZE = 512

# Calculated frequency resolution (Hz per FFT bin). Do not edit manually.
FREQ_BIN = SAMPLE_RATE / CHUNK_SIZE

# === NETWORK TIMING ===
# Minimum time (seconds) between sending UDP packets to bulbs.
# - 0.04s = 25 updates/sec. Good balance between speed and stability.
# - <0.03s: May cause bulbs to freeze or drop packets.
WIZ_UPDATE_INTERVAL = 0.04

# === FREQUENCY BANDS (Hz) ===
# Frequency ranges for analyzing the audio spectrum.
# - Bass: Controls brightness/strobe and color changes.
# - Mids/Highs: Currently visualization-only in console.
BASS_RANGE = (20, 250)
MID_RANGE = (250, 4000)
HIGH_RANGE = (4000, 20000)

# === VISUALIZATION PHYSICS (SMOOTHING) ===
# How fast the visualization bar jumps UP when a loud sound occurs (0.0 - 1.0).
# - 0.95: Almost instant reaction (good for kick drums).
# - 0.10: Very slow rise.
ATTACK_FACTOR = 0.95

# How fast the visualization bar falls DOWN when sound stops (0.0 - 1.0).
# - 0.15: Fast decay (snappy, stroboscopic effect).
# - 0.05: Slow, smooth fading trail.
RELEASE_FACTOR = 0.15

# Gamma correction exponent.
# - 1.0: Linear response.
# - 3.0: High contrast. Quiet sounds become invisible, loud sounds pop.
CONTRAST = 3.0

# Width of the ASCII bars in the console (number of characters).
BAR_WIDTH_MINI = 20

# === SIGNAL PROCESSING ===
# Noise Gate: Absolute raw energy level (0.0 - 1.0) considered as "silence".
# Any signal below this is ignored. Prevents the auto-gain algorithm
# from amplifying background hiss/noise to 100% during quiet parts of a song.
MIN_VOLUME_THRESHOLD = 0.002

# === LIGHT CONTROL LOGIC ===
# Brightness Cutoff (%):
# If bass intensity is below this value (0-100), the lights turn OFF (Blackout).
# Creates high-contrast gaps between beats.
LIGHT_CUTOFF_THRESHOLD = 20

# Max Brightness Trigger (%):
# If bass intensity is above this value (0-100), the lights go to 100% brightness.
LIGHT_MAX_THRESHOLD = 90

# Beat Detection Threshold (%):
# Bass level required to trigger a random color change event.
BEAT_TRIGGER_THRESHOLD = 80

# Beat Cooldown (seconds):
# Minimum time to wait before changing color again.
# Prevents seizure-inducing flickering on fast songs (e.g. 180 BPM drill/dnb).
BEAT_COOLDOWN = 0.4
...
class WizController:
    ...
    def _save_initial_states(self):
        ...
        for ip in self.ips:
            try:
                ...
                if "result" in resp:
                    self.original_states[ip] = resp["result"]
            except Exception:
                pass
    ...
    def _restore_initial_states(self):
        ...
        for ip, state in self.original_states.items():
            ...
            if "r" in state:
                params.update({"r": state["r"], "g": state["g"], "b": state["b"]})
            elif "temp" in state:
                params["temp"] = state["temp"]
            elif "sceneId" in state:
    ...
    def update(self, bass_val):
        ...
        if bass_val < LIGHT_CUTOFF_THRESHOLD:
            self.target_dimming = 0
        elif bass_val > LIGHT_MAX_THRESHOLD:
            self.target_dimming = 100
        else:
    ...
    def _randomize_color(self):
        ...
        self.target_r = int(r * 255)
        self.target_g = int(g * 255)
        self.target_b = int(b * 255)
        ...
        while self.running:
            ...
            if now - self.last_sent > self.min_interval:
                ...
                if is_blackout:
                    msg = {
                        "method": "setPilot",
                        "params": {"r": 0, "g": 0, "b": 0, "dimming": safe_dimming},
                    }
                else:
                    msg = {
                        "method": "setPilot",
...
def main():
    ...
    try:
        ...
        if not target_mic:
            ...
            print(
                f"You need to install '{AUDIO_DEVICE_NAME}' to route system audio to Python."
            )
            print(
                f"Install command: {C_GREEN}brew install --cask blackhole-2ch{C_RESET}"
            )
            return
        ...
        with target_mic.recorder(samplerate=SAMPLE_RATE) as recorder:
            while True:
                ...
                sys.stdout.write(
                    f"\r{bass_proc.draw_mini()}   {mid_proc.draw_mini()}   {high_proc.draw_mini()}"
                )
                sys.stdout.flush()
